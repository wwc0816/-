<template>
    <div class="business wrapDiv">
        <div class="applyad commonDiv">
            ...
            <Button class="applyBtn" @click="editBtn">修改广告</Button>
        </div>
    </div>
</template>
...
export default {
  data() {
    return {
      urlapi: this.$common.path + "/advertising/get",
      editapi: this.$common.path + "/advertising/edit",
      nationalitys: [
        {
          value: "中国",
          label: "中国"
  ...
  methods: {
    editBtn() {
      ...
      this.$http.post(this.editapi, this.$common.sort(this.data)).then(res => {
    ...
    getadinfo(){
      this.$http
        .get(this.urlapi, {
          params: this.$common.sort({advertisingId:this.$route.params.id})
        })
        .then(res => {
          if (res.data.status == 1) {
