<template>
    <div v-show="statedata.State||statedata.State == 0">
        <div class="account_top" v-if="statedata.State == 1">
            <section>
                <span>总资产折合</span>
                <span>{{totaldata.currencyAcount.toFixed(8)}} {{$store.state.name.currencyname}}</span>
                <span class="aoumnt colorgray">{{totaldata.cny.toFixed(2)}} CNY</span>
            </section>
            <section>
                <a @click="pageTo('merchant')">商家服务</a>
        ...
        <div class="nomerchant" v-else>
            <p class="title">如何开通聚合支付</p>
            <p>1、申请前必须拥有经过实名认证的CoinsUp账户。</p>
            <p>2、需要满足以下准入条件</p>
            <p>1 ）企业或个人用户均可申请；</p>
            <p>2 ）提供真实有效地信息；</p>
            <p>3 ）详情说明接入产品内容；</p>
            <p>4 ）根据产品要求提交资料，审核周期为1-3工作日；</p>
            <p>5 ）申请聚合支付需提交1000WE做为保证金（2019年6月1日前免费开通）；</p>
            <Button type="primary" style='display:block;width:240px;height:53px;margin: 60px auto;font-size:18px;' @click="pageTo('merchantApply')">立即申请</Button>
        </div>
        <!-- 充值 -->
        <Modal
...
export default {
  data() {
    return {
      ...
      rechangeapi:
        this.$common.coinspath + "/v1/user/coin/query/recharge_address",
